...
function renderGroups(){
  ...
  el.innerHTML=`<div class="gg">${entries.map(([gid,g])=>{const info=g.info||{};const mC=Object.keys(g.members||{}).length;const tot=Object.values(g.expenses||{}).reduce((s,e)=>s+N(e.amount),0);return`<div class="gc" style="cursor:pointer" onclick="openGroup('${gid}')"><div style="font-size:1.5rem;margin-bottom:.5rem">${info.icon||'ðŸ‘¥'}</div><div style="font-weight:700;font-size:1rem;margin-bottom:.25rem">${info.name||'Gruppo'}</div><div style="font-size:.8rem;color:var(--tx2);margin-bottom:.75rem">${info.description||''}</div><div style="display:flex;gap:1rem;font-size:.8rem"><div><div style="color:var(--tx3)">Membri</div><div style="font-weight:600">${mC}</div></div><div><div style="color:var(--tx3)">Spese</div><div style="font-weight:600;color:var(--re)">${fmt(tot)}</div></div></div></div>`;}).join('')}</div>`;
}
...
function openGroup(gid){
  ...
  document.getElementById('gd-expenses').innerHTML=expenses.slice(0,10).map(e=>`<tr><td>${e.payerName||'â€”'}</td><td class="tdm">${e.description||'â€”'}</td><td style="font-weight:700;color:var(--re)">${fmt(e.amount)}</td></tr>`).join('');
  ...
  chatOff=db.ref(`groups/${gid}/chat`).limitToLast(50).on('value',snap=>{
    ...
    chatEl.innerHTML=msgs.map(m=>{const mine=m.uid===UID;return`<div class="${mine?'chat-mine':'chat-other'}">${m.text}<div class="chat-meta">${mine?'Tu':m.name||'?'} Â· ${new Date(m.ts||0).toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'})}</div></div>`;}).join('');
